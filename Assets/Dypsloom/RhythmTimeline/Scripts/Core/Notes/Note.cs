...
namespace Dypsloom.RhythmTimeline.Core.Notes
{
	...
	public class NoteTriggerEventData
	{
		...
		public void SetBadTriggerData(InputEventData eventData, double dspTimeDiff, float dspTimeDiffPerc)
		{
			InputEventData = eventData;
			TriggerDspTime = DspTime.AdaptiveTime;
			DspTimeDifference = dspTimeDiff;
	...
	public interface INoteOnSceneGUIChange
	{
#if UNITY_EDITOR
	...
	public interface INoteRhythmClipOnSceneGUIChange
	{
		#if UNITY_EDITOR
		public bool RhythmClipOnSceneGUIChange(UnityEditor.SceneView sceneView, RhythmClip mainSelectedClip,
			List<RhythmClip> selectedClips);
		#endif
	}
	
	public interface INoteRhythmClipOnNoteInstanceChange
	{
#if UNITY_EDITOR
		public void RhythmClipOnNoteInstanceChange(Note previousNote, Note newNote, VisualElement root, RhythmClip mainSelectedClip,
			List<RhythmClip> selectedClips);
#endif
	}
	...
	public abstract class Note : MonoBehaviour
	{
		...
		[Tooltip("By default the activate time is the clip start, the deactivate time is clip end. This variable can add a time offset to the activate and deactivate events. Especially useful when using a FixedDuration Note Definition.")]
		[SerializeField] protected Vector2 m_ActivateDeactivateTimeOffset = new Vector2(0,0);
		...
		public double TimeFromActivate => CurrentTime - NoteStart;
		public double TimeFromDeactivate => CurrentTime - NoteEnd;

		public double NoteStart => m_RhythmClipData.ClipStart + NoteClipStartOffset;
		public double NoteEnd => m_RhythmClipData.ClipEnd + NoteClipEndOffset;
		public virtual float NoteClipStartOffset => m_ActivateDeactivateTimeOffset.x;
		public virtual float NoteClipEndOffset => m_ActivateDeactivateTimeOffset.y;
		...
		/// <summary>
		/// This function that returns the RhythmClipExtraNoteData Type to assign to the RhythmClip when added to the timeline.
		/// Override this to set a custom RhythmClipExtraNoteDataType for your custom note type.
		/// </summary>
		public virtual Type RhythmClipExtraNoteDataType => null;
		...
		protected virtual void InvokeNoteBadTriggerEvent(InputEventData eventData, double dspTimeDiff, float dspTimeDiffPerc)
		{
			m_NoteTriggerEventData.SetBadTriggerData(eventData,dspTimeDiff,dspTimeDiffPerc);
			InvokeNoteTriggerEvent();
		}
		...
		public virtual void TimelineUpdate(double globalClipStartTime, double globalClipEndTime)
		{
			...
			if (m_ActivateWithClip == false) {
				if (m_ActiveState == ActiveState.Active && (globalClipEndTime+m_ActivateDeactivateTimeOffset.y) >= 0) {
					DeactivateNote();
				} else if(m_ActiveState == ActiveState.PreActive && (globalClipStartTime+m_ActivateDeactivateTimeOffset.x) >= 0) {
					ActivateNote();
				}
			}
		...
		/// <summary>
		/// The note gets destroyed by the Rhythm Processor when the note goes out of its lifetime
		/// (by default controlled by the NoteClips and Rhythm Director SpawneTimeRange).
		/// </summary>
		public virtual void DestroyNote()
		{
			//It should be deactivated if was not triggered.
			if (m_ActiveState == ActiveState.Active) {
				DeactivateNote();
			}
