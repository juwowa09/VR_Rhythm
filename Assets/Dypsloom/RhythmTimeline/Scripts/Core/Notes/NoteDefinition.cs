...
namespace Dypsloom.RhythmTimeline.Core.Notes
{
	...
	public class NoteDefinition : ScriptableObject
	{
		...
		[FormerlySerializedAs("m_ScaledCrochetValue")]
		[Tooltip("The scale factor used when the Scaled Crochet option is selected. Or when Fixed time is selected.")]
		[SerializeField] protected float m_DurationScaler = 1.2f;
		...
		public virtual void SetClipDuration(RhythmClip rhythmClip, TimelineClip clip)
		{
			...
			if (m_ClipDuration == ClipDurationType.ScaledCrochet) {
				clip.duration = rhythmClip.RhythmClipData.RhythmDirector.Crochet * m_DurationScaler;
				return;
			}
			
			if(m_ClipDuration == ClipDurationType.FixedDuration) {
				clip.duration = m_DurationScaler;
				return;
			}
		}
	}
