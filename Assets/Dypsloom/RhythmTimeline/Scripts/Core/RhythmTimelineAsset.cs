...
namespace Dypsloom.RhythmTimeline.Core
{
    ...
    public class RhythmTimelineAsset : TimelineAsset
    {
        ...
        [NonSerialized] protected IReadOnlyList<RhythmTrack> m_RhythmTracks;

        public IReadOnlyList<RhythmTrack> RhythmTracks
        {
            get
            {
                if (m_RhythmTracks == null) {

                    var newtrackList = new List<RhythmTrack>();
                
                    var outputTracks = GetOutputTracks();

                    foreach (var track in outputTracks) {
                        if (track is RhythmTrack rhythmBeatTrack) {
                            newtrackList.Add(rhythmBeatTrack);
                        }
                    }

                    m_RhythmTracks = newtrackList;
                }

                return m_RhythmTracks;
            }
        }
        ...
        public bool TryGetRhythmTackWithID(int trackIndex, out RhythmTrack rhythmTrack)
        {
            rhythmTrack = null;
            foreach (var track in RhythmTracks) {
