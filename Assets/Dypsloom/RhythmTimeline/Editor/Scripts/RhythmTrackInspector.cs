...
namespace Dypsloom.RhythmTimeline.Editor
{
    ...
    public class RhythmTrackInspector : UIElementsInspector
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            m_AddOptionOption = new EnumField("Add Option",GenerateClipOverrideOption.ClearPreviousNotes);
        ...
        private void AutofillTrack()
        {
            ...
            switch ((GenerateClipOverrideOption)m_AddOptionOption.value) {
        ...
        protected virtual void AddNotes(float startTime, float endTime, double step, float noteLength, float noteSpacing)
        {
            ...
            for (double i = 0; i < stepCount; i++) {
                ...
                CreateRhythmClip(m_RhythmTrack, m_AutoFillNoteDefinitionField.value as NoteDefinition, clipStart, clipDuration);
            }
        }

        private void CreateRhythmClip(RhythmTrack rhythmTrack, NoteDefinition noteDefinition, double clipStart, double clipDuration)
        {
            var clip = rhythmTrack.CreateClip<RhythmClip>();
            //By default set the name to an hidden character
            clip.displayName = " ";
            var clipAsset = clip.asset as RhythmClip;
            clipAsset.SetNoteDefinition(noteDefinition);
